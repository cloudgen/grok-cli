...
| `test_grok_setup.sh` | `setup` / `update-grok` fetch xAI channel + artifact (fake curl; no public net; no `install.sh`); TTY keep / reinstall / Exit; Android wrapper `--kill-on-exit` / `proot-exit-reaper` / `-p` SIGKILL; heal stale wrapper on skip | **TP-VCLI-*** |
