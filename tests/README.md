...
| `test_domain_grok_cli.sh` | session gate + backup/sync-auth + sync-auth-from-remote (incl. TTY menu pick 4) + add-crontab + sudoers print + JSON grant + submit inbound | **TP-GROK-CLI-*** |
