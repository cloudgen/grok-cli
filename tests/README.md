...
| `test_grok_setup.sh` | `setup` fetches xAI channel + artifact (fake curl; no public net; no `install.sh`); Android wrapper `--kill-on-exit` / `-p` SIGKILL; heal stale wrapper on skip | **TP-VCLI-*** |
