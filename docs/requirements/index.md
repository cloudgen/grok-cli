...
**Updated:** 2026-09-07 (25 Active + 4 superseded; grok-setup 2.8.0 `run`)
...
| requirement-grok-setup | Type 0 `setup` + `run`: studied xAI channel + artifact procedure so peer `grok` is installed; `run` starts grok without auto-update; Android wrapper `--kill-on-exit` / `-p` SIGKILL; heal stale wrapper on skip | domain | Active (2.8.0) | `requirement-grok-setup.md` | 2026-09-07 |
...
| requirement-shell-cli-interface | Shell CLI interface; `setup`; `run` (start grok without auto-update); auth verbs; `version-check` / `self-update` / `self-uninstall`; empty argv TTY menu / off-TTY ensure; overlay `--debug` follows 0-argv; `--json` no-command is 0-argv special case (JSON help even TTY); `--debug` menu elapsed dual mention | shell | Active (2.8.0) | `requirement-shell-cli-interface.md` | 2026-09-07 |
...
| requirement-shell-cli-default-interaction | Case 3 TTY empty argv + `menu`/`main` numbered list; this-login-only hosts list `run` first then hide backup/sync-auth/sudoers; logged-in hide sync-auth / sync-auth-from-remote; session line from live `grok -p hello`; `--debug` elapsed | shell | Active (2.9.0) | `requirement-shell-cli-default-interaction.md` | 2026-09-07 |
...
| requirement-domain-grok-cli | Domain **surface** SSOT (four pillars); ops defer to grok-auth-backup / grok-crontab; `setup` channel+artifact; `run` starts grok without auto-update; `check-session` is live `grok -p hello` | domain | Active (1.5.0) | `requirement-domain-grok-cli.md` | 2026-09-07 |
