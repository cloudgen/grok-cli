...
**Updated:** 2026-09-04 (24 Active + 4 superseded)
...
| requirement-grok-auth-backup | **Auth ops SSOT**: session gate + deposit `auth.*` to `/var/grok-cli` (root:root 0644) + unprivileged `sync-auth` / `sync-auth-from-remote` | backup | Active (1.1.3) | `requirement-grok-auth-backup.md` | 2026-09-03 |
...
| requirement-shell-cli-interface | Shell CLI interface; `setup`; auth verbs; `version-check` / `self-update` / `self-uninstall`; empty argv TTY menu / off-TTY ensure | shell | Active (2.7.5) | `requirement-shell-cli-interface.md` | 2026-09-04 |
...
| requirement-shell-cli-default-interaction | Case 3 TTY empty argv + `menu`/`main` numbered list; default CLI main menu style; header short desc; session line; off-TTY `menu` stays help; empty argv off-TTY is Type O | shell | Active (2.3.0) | `requirement-shell-cli-default-interaction.md` | 2026-09-03 |
...
| requirement-shell-termux-coding | Termux/Android host writing: honor `PREFIX`, Termux `pkg` (no sudo), `noexec` tmp, FHS-not-assumed; points at setup/folder/storage | shell | Active (1.0.0) | `requirement-shell-termux-coding.md` | 2026-09-04 |
...
| requirement-domain-grok-cli | Domain **surface** SSOT (four pillars); ops defer to grok-auth-backup / grok-crontab; `setup` channel+artifact | domain | Active (1.4.5) | `requirement-domain-grok-cli.md` | 2026-09-04 |
