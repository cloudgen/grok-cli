...
**Updated:** 2026-09-07 (25 Active + 4 superseded; grok-setup 2.7.0 wrapper hang)
...
| requirement-grok-auth-backup | **Auth ops SSOT**: live `grok -p hello` session gate (before any `auth.json` parse; always bounded, GNU `timeout -k` or watchdog) + deposit `auth.*` to `/var/grok-cli` (root:root 0644) + unprivileged `sync-auth` / `sync-auth-from-remote` (skip when already logged in; preferred SPEC in persistence); elevated probe uses `SUDO_USER` home | backup | Active (1.5.0) | `requirement-grok-auth-backup.md` | 2026-09-07 |
| requirement-grok-setup | Type 0 `setup`: studied xAI channel + artifact procedure so peer `grok` is installed (MUST NOT fetch/exec `install.sh`; smoke from `{{GROK_HOME}}/downloads`; skip only if grok runs on this host; refuse wrong ELF; Android ET_EXEC wrapper with `--kill-on-exit` and `grok -p` SIGKILL; heal stale wrapper on skip; Termux proot + resolv bind; HTTP status on curl fail; stale session PATH is not an error) | domain | Active (2.7.0) | `requirement-grok-setup.md` | 2026-09-07 |
...
| requirement-shell-cli-interface | Shell CLI interface; `setup`; auth verbs; `version-check` / `self-update` / `self-uninstall`; empty argv TTY menu / off-TTY ensure; overlay `--debug` follows 0-argv; `--json` no-command is 0-argv special case (JSON help even TTY); `--debug` menu elapsed dual mention | shell | Active (2.7.9) | `requirement-shell-cli-interface.md` | 2026-09-07 |
...
| requirement-shell-cli-default-interaction | Case 3 TTY empty argv + `menu`/`main` numbered list; overlay `--debug` with no command is TTY empty argv; TTY `--json` no command is JSON help (0-argv special case, not this menu); default CLI main menu style; header short desc; session line from live `grok -p hello` (must not freeze Termux); this-login-only hosts hide backup/sync-auth/sudoers and print not-available line; logged-in session hides sync-auth / sync-auth-from-remote and **appends** logged-in not-available; `--debug` elapsed of each paint step; off-TTY `menu` stays help; empty argv off-TTY is Type O | shell | Active (2.8.2) | `requirement-shell-cli-default-interaction.md` | 2026-09-07 |
...
| requirement-shell-termux-coding | Termux/Android host writing: honor `PREFIX`, Termux `pkg` (no sudo), `noexec` tmp, FHS-not-assumed; session probe exec always bounded (`timeout -k` / watchdog); Android wrapper `--kill-on-exit` + `grok -p` SIGKILL (points at setup 18e) | shell | Active (1.3.0) | `requirement-shell-termux-coding.md` | 2026-09-07 |
