...
**Updated:** 2026-09-10 (26 Active + 4 superseded; `update-grok`)
...
| requirement-grok-setup | Type 0 `setup` + `update-grok` + `run`: studied xAI channel + artifact so peer `grok` is installed or refreshed; Git Bash/MSYS/Cygwin `windows-*` PE; `run` starts grok without auto-update; Android wrapper `--kill-on-exit` / `proot-exit-reaper` / `start-services.sh` no-op bind; heal stale wrapper on skip **and** `self-update` | domain | Active (2.13.0) | `requirement-grok-setup.md` | 2026-09-10 |
...
| requirement-shell-cli-interface | Shell CLI interface; `setup`; `update-grok`; `run` (start grok without auto-update); auth verbs; `version-check` / `self-update` / `self-uninstall` (start line names local and remote VERSION); empty argv TTY menu / off-TTY ensure; overlay `--debug` follows 0-argv; `--json` no-command is 0-argv special case (JSON help even TTY); `--debug` menu elapsed dual mention; `rc-test` dual mention | shell | Active (2.10.0) | `requirement-shell-cli-interface.md` | 2026-09-10 |
...
| requirement-domain-grok-cli | Domain **surface** SSOT (four pillars); ops defer to grok-auth-backup / grok-crontab; `setup` / `update-grok` channel+artifact; `run` starts grok without auto-update; `check-session` is live `grok -p hello` (`proot` → reaper else simple `-p`) | domain | Active (1.7.0) | `requirement-domain-grok-cli.md` | 2026-09-10 |
