...
| requirement-shell-cli-default-interaction | Case 3 TTY empty argv + `menu`/`main` numbered list; default CLI main menu style; header short desc; session line from live `grok -p hello` (must not freeze Termux); this-login-only hosts hide backup/sync-auth/sudoers and print not-available line; logged-in session hides sync-auth / sync-auth-from-remote and **appends** logged-in not-available; off-TTY `menu` stays help; empty argv off-TTY is Type O | shell | Active (2.7.0) | `requirement-shell-cli-default-interaction.md` | 2026-09-07 |
