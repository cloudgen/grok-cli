...
**Updated:** 2026-09-12 (26 Active + 4 superseded; TTY `setup` reinstall)

| ID / key | Title | Area | Status | Path | Updated |
|----------|-------|------|--------|------|---------|
| requirement-class-software-dev | Software-development class law + leftover stack (posix-sh, dual-mode); no dest approver / no dest fence; leftover PATH/profile → path-and-shell-support | class | Active (1.3.4) | `requirement-class-software-dev.md` | 2026-09-09 |
...
| requirement-grok-auth-backup | **Auth ops SSOT**: session gate — PRoot/Termux local `auth.json` cookies (no live `grok -p hello`); other hosts live `grok -p hello` (bounded, default 14s; timeout is not logged-out; live probe code protected) + deposit `auth.*` to `/var/grok-cli` (root:root 0644) + unprivileged `sync-auth` / `sync-auth-from-remote` (skip when already logged in; preferred SPEC in persistence); elevated probe uses `SUDO_USER` home | backup | Active (1.9.0) | `requirement-grok-auth-backup.md` | 2026-09-07 |
| requirement-grok-setup | Type 0 `setup` + `update-grok` + `run`: studied xAI channel + artifact so peer `grok` is installed or refreshed; TTY already-installed keep / reinstall / Exit; Git Bash/MSYS/Cygwin `windows-*` PE; `run` starts grok without auto-update; Android wrapper `--kill-on-exit` / `proot-exit-reaper` / `start-services.sh` no-op bind; heal stale wrapper on skip **and** `self-update` | domain | Active (2.14.0) | `requirement-grok-setup.md` | 2026-09-12 |
...
| requirement-shell-cli-interface | Shell CLI interface; `setup` (TTY reinstall list); `update-grok`; `run` (start grok without auto-update); auth verbs; `version-check` / `self-update` / `self-uninstall` (start line names local and remote VERSION); empty argv TTY menu / off-TTY ensure; overlay `--debug` follows 0-argv; `--json` no-command is 0-argv special case (JSON help even TTY); `--debug` menu elapsed dual mention; `rc-test` dual mention | shell | Active (2.11.0) | `requirement-shell-cli-interface.md` | 2026-09-12 |
...
| requirement-shell-cli-default-interaction | Case 3 TTY empty argv + `menu`/`main` numbered list; this-login-only hosts list `run` first then hide backup/sync-auth/sudoers; logged-in hide sync-auth / sync-auth-from-remote; session and not-available lines are independent `[INFO]`; PRoot/Termux local auth cookies else live probe + checking-session; `--debug` elapsed | shell | Active (2.14.0) | `requirement-shell-cli-default-interaction.md` | 2026-09-09 |
| requirement-shell-path-and-shell-support | Shell-rc PATH + profile ensure (sibling unify, scoped uninstall, heal, routed `rc-test`) | shell | Active (1.0.0) | `requirement-shell-path-and-shell-support.md` | 2026-09-09 |
...
| requirement-domain-grok-cli | Domain **surface** SSOT (four pillars); ops defer to grok-auth-backup / grok-crontab; `setup` / `update-grok` channel+artifact; TTY `setup` reinstall list; `run` starts grok without auto-update; `check-session` is live `grok -p hello` (`proot` → reaper else simple `-p`) | domain | Active (1.8.0) | `requirement-domain-grok-cli.md` | 2026-09-12 |
