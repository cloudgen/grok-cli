...
| L-WRAP-03 | Leftover discovery on PRoot was inexact: grok-linux already exited; PRoot cannot determine that (`do_wait`); `comm` truncates to `/data/data/com.` | Dispatch: `proot` on PATH → reaper (args/exe; new `runsvdir` only; SIGKILL PRoot); else simple `grok -p hello`; TP-VCLI-29/30 · TP-GROK-CLI-47/48; **INC-20260907-003** | open watch |
