...
| L-MENU-04 | After 1.8.23, Termux `grok-cli` still looks hung after the INFO header: live `session` probe (~14s) has no non-DEBUG progress; `DEBUG=1` is the same path; Ctrl-Z stops grok-cli **and** the PRoot reaper (same PGID); empty stdout never trips idle | Non-DEBUG progress before `grok -p hello`; `setsid` collector+reaper; trap INT/TSTP and SIGKILL; guest-gone / idle (incl. empty stdout) SIGKILL PRoot now; TP-CLI-21/23/26 · TP-GROK-CLI-49; **INC-20260907-004** | open watch |
| L-WRAP-01 | Termux operator-facing `grok -p` prints then hangs until Ctrl-Z (`exec proot` without `--kill-on-exit`; SIGINT ignored; `setup` skip never rewrote the wrapper) | Wrapper: `proot --kill-on-exit` (never `-k`); `-p` `--no-auto-update` + SIGKILL on INT; heal stale wrapper on skip; TP-VCLI-26/27/28; **INC-20260907-002** | open watch |
| L-WRAP-04 | `grok-cli self-update` to 1.8.23 does not rewrite `~/.grok/bin/grok`; host kept the 2026-09-04 436-byte launcher (no `--kill-on-exit` / start-services bind / `proot-exit-reaper`); leftovers then `os error 11` | Heal Android wrapper on **self-update** (already-at-remote too) and on the session probe; TP-VCLI-32; **INC-20260907-004** | open watch |
| L-WRAP-03 | Leftover discovery on PRoot was inexact: grok-linux already `_exit`’d; leftover `runsvdir` from `bash -lc` → `start-services.sh` keep `wait4(-1,__WALL)` from draining; `comm` truncates | Bind `/dev/null` over `${PREFIX}/etc/profile.d/start-services.sh`; reaper as safety net; TP-VCLI-29/30/31 · TP-GROK-CLI-47/48; **INC-20260907-003** | open watch |
| L-WRAP-02 | Operator types bare `grok` / `grok -p` on Termux and hangs; no grok-cli verb starts grok with auto-update off | Routed **`run`**: inject `--no-auto-update`; this-login-only menu lists it first; `--json` does not exec; missing grok Next `setup`; TP-GROK-CLI-46 · TP-CLI-19/20 | open watch |
