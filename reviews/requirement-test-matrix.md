...
**Updated:** 2026-09-07 (1.8.24 Termux session probe / wrapper heal)  
...
| requirement-grok-auth-backup | backup | TP-GROK-CLI-03..10, 12 · 30..45 · 47 · 48 · 49 | Live `grok -p hello` session gate (before `auth.json`); always bounded; `proot` on PATH → TSTP-safe reaper else simple `-p`; elevated probe uses `SUDO_USER` home; grant-present child fail is not missing sudoers; deposit; sync-auth; sync-auth-from-remote; skip sync when already logged in; preferred SPEC in persistence; TTY menu SPEC prompt; production dest fail-closed |
| requirement-grok-setup | domain | TP-VCLI-01..09 · 11..32 · TP-CLI-04 · TP-CLI-13 · TP-GROK-CLI-46 | Peer grok channel + artifact; Android ET_EXEC wrapper; Termux proot `--kill-on-exit` + `proot-exit-reaper` + `start-services.sh` no-op bind; heal stale wrapper on skip **and** `self-update`; `run` starts grok without auto-update |
