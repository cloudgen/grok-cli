...
| setup | `gc_setup` | you (Type 0) | 2026-09-02 | `setup: Install grok from x.ai (channel + artifact; skip if present)` |
...
A TTY main menu **MUST** print daily-work **human-readable** lines as a **numbered list**, with related grant/draft verbs behind family row **`sudoers`** (submenu; **Back 8**, **Exit 9**). grok-cli is **case 3**: verb **`menu`** (alias **`main`**). Off-TTY empty argv is Type O ensure (not this list). **Exclude** `help`, `menu`/`main`, install/setup, self-managed (`install`, `uninstall`, `self-update`, `self-uninstall`, `where-is-me`), diagnostics (`version`, `about`, `version-check`), and test-purpose. **`sudoers` is not a live dispatcher token.** Main **N = 6**; submenu **K = 5**; **Exit 9**. Internal `ensure` is not a help verb.
